                ...
                Value::String(request_string) => {
                    match request_string.as_str() {
                        "motor" => {
                            ...
                            let converted_drive: Vec<_> = drive
                                .iter()
                                .filter_map(|value| Some(value.as_i64()? as i32))
                                .collect();
                            if converted_drive.len() == 2 {
                                (&self.to_motor_sender)
                                    .send(converted_drive)
                                    .ok()
                                    .ok_or(())?;
                            } else {
                                return Err(());
...
fn main() {
    ...
    ultrasonic::init_ultrasonic(
        gpio.clone(),
        to_client_message_sender.clone(),
        timer.clone(),
    );
        ...
        loop {
            if let Ok(received_message) = to_client_message_receiver.recv() {
                    ...
                    < timer.elapsed().as_nanos() as u64
                {
                    ...
                    let mut i = 0;
                    // Removes senders that have returned errors
                    for client in unlocked_client_senders.iter() {
                        if let Err(_) = client.send(ws::Message::Text(
                            serde_json::to_string(&received_message).unwrap_or(String::new()),
                        )) {
...
fn _test_motor(to_motor_sender: Sender<Vec<i32>>) {
    to_motor_sender
        .send(vec![100, 100])
        .expect("Failed to start motors.");
    spin_sleep::sleep(Duration::from_millis(5000));
    to_motor_sender
        .send(vec![50, 0])
        .expect("Failed to change motor direction.");
    spin_sleep::sleep(Duration::from_millis(5000));
    to_motor_sender
        .send(vec![0, 0])
        .expect("Failed to stop motors.");
}
